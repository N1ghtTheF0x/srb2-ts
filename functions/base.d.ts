...
declare type HookMap = {
	"IntermissionThinker": () => void
	"KeyDown": (keyevent: keyevent_t) => boolean
	"KeyUp": (keyevent: keyevent_t) => boolean
	"LinedefExecute": (line: line_t,mobj: mobj_t,sector: sector_t) => void
	"MapChange": (mapnum: int) => void
	"MapLoad": (mapnum: int) => void
	"ThinkFrame": () => void
	"PreThinkFrame": () => void
	"PostThinkFrame": () => void
	"GameQuit": () => void
	"BotAI": (player: mobj_t,bot: mobj_t) => LuaMultiReturn<[boolean,boolean,boolean,boolean,boolean,boolean,boolean,boolean]> | {forward: boolean,backward: boolean, left: boolean,right: boolean,strafeleft: boolean,straferight: boolean,jump: boolean,spin: boolean}
	"BotTiccmd": (bot: player_t,cmd: ticcmd_t) => boolean
	"BotRespawn": (player: player_t,bot: mobj_t) => boolean
	"HurtMsg": (player: player_t,inflictor: mobj_t,source: mobj_t) => boolean
	"NetVars": (network: <Arg>(sync: Arg) => Arg) => void
	"PlayerJoin": (playernum: int) => void
	"PlayerMsg": (source: player_t,type: int,target: player_t,msg: string) => boolean
	"PlayerQuit": (player: player_t,reason: int) => void
	"TeamSwitch": (player: player_t,team: int,fromspectators: boolean,autobalance: boolean,scramble: boolean) => boolean | nil
	"ViewpointSwitch": (player: player_t,nextviewedplayer: player_t,forced: boolean) => boolean | nil
	"SeenPlayer": (player: player_t,seenplayer: player_t) => boolean | nil
	"AbilitySpecial": (player: player_t) => boolean
	"JumpSpecial": (player: player_t) => boolean
	"JumpSpinSpecial": (player: player_t) => boolean
	"SpinSpecial": (player: player_t) => boolean
	"BossDeath": (mobj: mobj_t) => boolean
	"BossThinker": (mobj: mobj_t) => boolean
	"FollowMobj": (player: player_t,followmobj: mobj_t) => boolean
	"MapThingSpawn": (mobj: mobj_t,mapthing: mapheader_t) => boolean
	"MobjCollide": (thing: mobj_t,tmthing: mobj_t) => boolean
	"MobjLineCollide": (mobj: mobj_t,line: line_t) => boolean
	"MobjDamage": (target: mobj_t,inflictor: mobj_t,source: mobj_t,damage: int,damagetype: int) => boolean
	"MobjDeath": (target: mobj_t,inflictor: mobj_t,source: mobj_t,damagetype: int) => boolean
	"MobjFuse": (mobj: mobj_t) => boolean
	"MobjMoveBlocked": (mobj: mobj_t,thing: mobj_t,line: line_t) => boolean
	"MobjMoveCollide": (tmthing: mobj_t,thing: mobj_t) => boolean
	"MobjRemoved": (mobj: mobj_t) => boolean
	"MobjSpawn": (mobj: mobj_t) => boolean
	"MobjThinker": (mobj_t: mobj_t) => boolean
	"PlayerThink": (player: player_t) => void
	"PlayerCanDamage": (player: player_t,mobj: mobj_t) => boolean
	"PlayerSpawn": (player: player_t) => void
	"PlayerCmd": (player: player_t,cmd: ticcmd_t) => void
	"PlayerHeight": (player: player_t) => fixed_t
	"PlayerCanEnterSpinGaps": (player: player_t) => boolean
