...
const declareConstant = (name: string) => `declare const ${name}: number\n`
...
function convert()
{
    ...
    for(var index = 0;index < Convert.Count;index++)
    {
        const name = Convert[index]
        constants.push(declareConstant(name))
    }
    return constants.join("")
}

const SHORT_NAME = "sfx"
const FILE_NAME = `${SHORT_NAME}.d.ts`

const CONSTANTS_FOLDER = resolve(process.cwd(),"constants")
const FILE_PATH = resolve(CONSTANTS_FOLDER,FILE_NAME)

writeFileSync(FILE_PATH,convert(),"utf-8")